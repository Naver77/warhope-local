...
export default function ProductDetail() {
  ...
  // Mendukung folder [id] atau [slug]
  const id = params?.id || params?.slug;

  const [product, setProduct] = useState(null);
  const [selectedColor] = useState("Default"); // Dibuat statis karena MVP hanya 1 warna
      ...
      try {
        ...
        if (foundProduct) {
          ...
          if (typeof foundProduct.sizes === "string") {
            try { sizeObj = JSON.parse(foundProduct.sizes); } catch {}
          } else if (foundProduct.sizes && typeof foundProduct.sizes === "object") {
            sizeObj = foundProduct.sizes;
          }
          
          setAvailableSizes(sizeObj);
          ...
          <div className="lg:col-span-7 flex flex-col gap-8">
            ...
            <div className="bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800 rounded-4xl p-6 md:p-8 shadow-sm flex flex-col">
              <div className="flex overflow-x-auto hide-scrollbar gap-6 border-b border-slate-200 dark:border-slate-700 mb-6 pb-2 shrink-0">
                <button type="button" onClick={() => setActiveTab("deskripsi")} className={`pb-2 whitespace-nowrap text-sm font-bold transition-colors relative ${activeTab === "deskripsi" ? "text-blue-600" : "text-foreground/50"}`}>Deskripsi {activeTab === "deskripsi" && <span className="absolute bottom-0 left-0 w-full h-0.5 bg-blue-600 rounded-t-full -mb-2.5"></span>}</button>
                <button type="button" onClick={() => setActiveTab("spesifikasi")} className={`pb-2 whitespace-nowrap text-sm font-bold transition-colors relative ${activeTab === "spesifikasi" ? "text-blue-600" : "text-foreground/50"}`}>Spesifikasi {activeTab === "spesifikasi" && <span className="absolute bottom-0 left-0 w-full h-0.5 bg-blue-600 rounded-t-full -mb-2.5"></span>}</button>
                <button type="button" onClick={() => setActiveTab("ulasan")} className={`pb-2 whitespace-nowrap text-sm font-bold transition-colors relative flex items-center gap-2 ${activeTab === "ulasan" ? "text-blue-600" : "text-foreground/50"}`}>Ulasan <span className="bg-slate-100 dark:bg-slate-700 text-[10px] px-2 py-0.5 rounded-full">{reviews.length}</span>{activeTab === "ulasan" && <span className="absolute bottom-0 left-0 w-full h-0.5 bg-blue-600 rounded-t-full -mb-2.5"></span>}</button>
              </div>
                ...
                {activeTab === "ulasan" && (
                  <div className="animate-in fade-in space-y-8">
                    ...
                    <div className="bg-slate-50 dark:bg-slate-900/40 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 flex flex-col md:flex-row items-center gap-6">
                      <div className="text-center md:text-left">
                        ...
                        <div className="flex text-amber-400 mt-2 mb-1 justify-center md:justify-start">
                          {[1, 2, 3, 4, 5].map((star) => (
                            <Star key={star} className={`w-4 h-4 ${star <= Math.round(averageRating) ? 'fill-current' : 'text-slate-200 dark:text-slate-700'}`} />
                          ))}
                        </div>
                      ...
                      <div className="flex-1 text-xs text-slate-500 bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-100 dark:border-slate-700">
                        <span className="font-bold text-blue-600 dark:text-blue-400">Penting:</span> Untuk menjaga keaslian ulasan, hanya pelanggan yang telah membeli dan menerima produk ini (Verified Buyer) yang dapat memberikan penilaian melalui halaman Riwayat Pesanan.
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      {isLoadingReviews ? (
                        <div className="text-center py-6 text-slate-400"><div className="w-5 h-5 border-2 border-blue-600 border-t-transparent rounded-full animate-spin mx-auto"></div></div>
                      ) : reviews.length === 0 ? (
                              ...
                              <div className="flex justify-between items-start gap-4 mb-2">
                                <div>
                                  <p className="font-bold text-foreground text-sm flex items-center gap-1.5">
                                    {review.user_name} 
                                    <span className="bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 text-[8px] px-1.5 py-0.5 rounded uppercase tracking-wider flex items-center gap-0.5">
                                      <ShieldCheck className="w-2 h-2" /> Pembeli
                                    </span>
                                  </p>
          ...
          <div className="lg:col-span-5 flex flex-col justify-start">
            <div className="sticky top-28 bg-white dark:bg-slate-800/30 p-6 md:p-8 rounded-4xl border border-transparent dark:border-slate-800 shadow-sm">
              <div className="mb-8">
                ...
                <div className={`mt-2 inline-flex items-center gap-2 px-3 py-1 text-xs font-bold rounded-full ${currentMaxStock > 0 ? 'bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400' : 'bg-red-50 text-red-600 dark:bg-red-900/30 dark:text-red-400'}`}>
                      ...
                      return (
                        <button
                          ...
                          className={`h-12 rounded-xl flex items-center justify-center text-sm font-bold transition-all border 
                            ${selectedSize === size ? "border-blue-600 bg-blue-600 text-white shadow-md" : 
                            isOutOfStock ? "border-slate-100 bg-slate-50 text-slate-300 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-600 cursor-not-allowed" : 
                            "border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800 text-foreground hover:border-slate-400 dark:hover:border-slate-500"}`}
                        >
                          {size}
                        </button>
                      );
