...
import { Heart, ShoppingCart, Trash2, ArrowLeft, ChevronRight, Tag } from 'lucide-react';
...
// ----------------------------------------------------------------------
const WishlistItem = ({ product, index, onRemove, onMoveToCart, formatRupiah }) => {
  ...
  return (
    <div className="relative w-full rounded-2xl mb-4 bg-red-500 overflow-hidden group">
      ...
      >
        ...
        {/* Gambar Produk */}
        <Link href={`/product/${product.id}`} className="relative w-20 h-20 sm:w-24 sm:h-24 bg-slate-100 dark:bg-slate-900 rounded-xl overflow-hidden shrink-0 border border-slate-100 dark:border-slate-800 block">
          <Image
        ...
        <div className="flex-1 min-w-0 py-1">
          <Link href={`/product/${product.id}`}>
            <p className="text-[10px] sm:text-xs font-bold text-foreground/40 uppercase tracking-widest mb-0.5">{product.category}</p>
      ...
      {wishlist.length === 0 ? (
        <div className="bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800 rounded-4xl p-12 text-center flex flex-col items-center justify-center min-h-[50vh] shadow-sm animate-in fade-in zoom-in-95 duration-500">
          ...
          <p className="text-foreground/60 mb-8 max-w-md mx-auto">
            Anda belum menyimpan produk apa pun. Temukan gaya favorit Anda di katalog dan simpan di sini untuk dibeli nanti!
          </p>
          <Link 
            href="/katalog"
