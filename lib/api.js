...
export const deleteProduct = async (id) => {
  try {
    // 🔥 TAMBAHAN: Kita tambahkan .select() di akhir
    const { data, error } = await supabase
      .from("products")
      .delete()
      .eq("id", id)
    ...
    // 🔥 PENGAMAN BARU: Jika array kosong, berarti data gagal dihapus (kemungkinan diblokir RLS)
    if (!data || data.length === 0) {
      throw new Error("Gagal menghapus! Akses ditolak oleh sistem keamanan (RLS) atau produk tidak ditemukan.");
    }

    return true;
  } catch (error) {
    console.error("Detail Error Hapus Produk:", error);
    
    if (error.code === '23503') {
      throw new Error("Gagal menghapus! Produk ini sudah memiliki riwayat transaksi atau sedang berada di keranjang pelanggan.");
    }
    
    throw new Error(error.message || "Gagal menghapus produk karena alasan sistem.");
  }
};
...
export const createOrder = async (orderPayload, cartItems) => {
  ...
  try {
    ...
    for (const item of cartItems) {
      ...
      // ✅ VALIDASI KEAMANAN: Pastikan ID tidak kosong sebelum dikirim ke DB
      if (!item.id) {
        throw new Error(`Data keranjang rusak. ID kosong pada produk: ${item.name}`);
      }

      console.log(`Mengirim perintah potong stok ke Supabase untuk ID: ${item.id}`);
...
export const getUserProfile = async (userId, retries = 3) => {
  try {
    const { data, error } = await supabase
      ...
      .eq("id", userId)
      .maybeSingle(); // ✅ PERBAIKAN: Ganti .single() menjadi .maybeSingle()
...
export const getAdminOrdersSummary = async (limit = null) => {
  try {
    ...
    let query = supabase
      .from("orders")
      .select("id, invoice_number, created_at, status, total_amount, customer_name, customer_email, payment_method")
      .order("created_at", { ascending: false });
