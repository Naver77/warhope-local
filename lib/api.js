...
export const getUserProfile = async (userId, retries = 3) => {
  try {
    ...
    // Tangkap error Lock atau Abort seperti di getAllOrders
    if (error.message?.includes("Lock broken") || error.name === "AbortError" || error.message?.includes("steal")) {
      if (retries > 0) {
        // Tunggu 500ms lalu coba lagi secara diam-diam
        await new Promise(resolve => setTimeout(resolve, 500)); 
        return getUserProfile(userId, retries - 1);
      }
    }
    
    // Jika benar-benar error selain Lock
